...
d=json.loads((OUT/'design.json').read_text());scene=bpy.data.scenes.new('WonderForge — long-load master link');bpy.context.window.scene=scene
...
for i,s in enumerate(d['strands']):curve(f'lower-rope-eye-{i}',s['eye']['points']);curve(f'sling-leg-{i}',s['line'])
...
bpy.ops.export_scene.gltf(filepath=str(OUT/'model/master-link.glb'),export_format='GLB',use_selection=True,use_active_scene=True,export_extras=True,export_yup=True,export_animations=False)
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'blender/eiffel-master-link.blend'))
result={'scene':scene.name,'objects':len(scene.objects),'blend':str(OUT/'blender/eiffel-master-link.blend'),'productionReady':False}
