...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'artifacts/eiffel-first-floor-winch-2026-09-08';OUT.mkdir(exist_ok=True)
shapes=[]
...
for x in (-.8,.8):
 ...
 beam(f'mast-{x}',[x,197.16,-3.4],[x,202.4,-3.4],.085)
 beam(f'back-stay-{x}',[x,197.24,-5.9],[x,202.4,-3.4],.07)
 beam(f'head-arm-{x}',[x,202.4,-3.4],[x,202.4,-2.05],.085)
 beam(f'head-brace-{x}',[x,198.9,-3.4],[x,202.4,-2.05],.07)
for z in (-5.9,-5.15,-4.1):box(f'base-cross-{z}',[0,197.24,z],[1.76,.16,.16])
for z in (-3.4,-2.05):
 for sign in (-1,1):box(f'rail-outrigger-{z}-{sign}',[sign*.6075,202.4,z],[.555,.16,.17])
for x in (-.35,.35):box(f'trolley-rail-{x}',[x,202.54,-2.95],[.10,.12,2.50])
box('winch-bed',[0,197.39,-5.5],[1.5,.14,.85])
for x in (-.55,.55):box(f'drum-bearing-{x}',[x,197.73,-5.5],[.12,.54,.2])
design=dict(floorY=197,productionReady=False,frame=shapes,drum=dict(center=[0,197.8,-5.5],radius=.30,width=.95,axis=0),sheave=dict(center=[0,202.06,-2.05],radius=.25,width=.16,axis=0),cargoLane=[0,-1.8],trolley=dict(startZ=-2.05,endZ=-3.85,railTop=202.6,wheelRadius=.115,wheelAxisY=202.715),limits=['Interpreted first-floor freight head-frame; canonical coordinates are transformed rigidly to the verified first-floor landing.','Skids contact the actual first-floor deck; capacity, hold-down fastening and erection remain unverified.','Steam drive and operational cargo handoff are not yet modeled.'])
