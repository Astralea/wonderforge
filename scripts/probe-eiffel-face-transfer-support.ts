...
const anchors=[{id:'east-saddle',point:[51,15.84,-44.5]}, {id:'west-saddle',point:[46.6,15.84,-42.8]}, {id:'east-raker',point:[50.9,17.3,-42.2]}, {id:'west-raker',point:[48.8,17.3,-41.8]}];
const reports=[];
for(const c of search.candidates.slice(0,25)){
 ...
 const choices=ends.map((b,i)=>anchors.map(anchor=>{
  ...
  const hits=clashes(eiffelConvexBox(eiffelReceiverBeamBox(a,b,.08)));
  return {...beam,anchor:anchor.id,length:Math.hypot(...a.map((v,k)=>v-b[k]!)),hits};
 }).filter(b=>b.length<5.8));
 reports.push({...c,choices});
}
writeFileSync('artifacts/eiffel-face-transfer-2026-09-08/support-search.json',JSON.stringify(reports,null,2)+'\n');
for(const [i,r] of reports.entries())console.log(i, r.seat.position,r.choices.map(c=>c.map(b=>({anchor:b.anchor,hits:b.hits.map(h=>h.fixed)}))));
