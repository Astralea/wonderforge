...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'artifacts/eiffel-face-transfer-2026-09-08'
shapes=json.loads((OUT/'receiver-occupancy.json').read_text());design=json.loads((OUT/'receiver-design.json').read_text())
scene=bpy.data.scenes.new('WonderForge — supported face-package receiver');bpy.context.window.scene=scene
collection=bpy.data.collections.new('Independent receiving deck');scene.collection.children.link(collection)
...
base.name='receiver-joined-timber';base['wf_role']='receiver-frame';base['wf_source_occupancy_sha256']=hashlib.sha256((OUT/'receiver-occupancy.json').read_bytes()).hexdigest()
...
bpy.ops.export_scene.gltf(filepath=str(OUT/'model/receiver.glb'),export_format='GLB',use_selection=True,use_active_scene=True,export_extras=True,export_yup=True,export_animations=False,export_lights=False,export_cameras=False)
result={'scene':scene.name,'blend':str(OUT/'blender/eiffel-face-receiver.blend'),'sourceOccupancySHA256':base['wf_source_occupancy_sha256'],'productionAdmitted':False,'limits':['Timber bearing geometry study. Fixings, rated strength, worker access and erection are not reconstructed.','Hidden source prisms are retained for editing; exported joined mesh is the visible union.']}
