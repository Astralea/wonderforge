...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'artifacts/eiffel-summit-operation-2026-09-08'
EXT=Path.home()/'Library/Application Support/Claude/Claude Extensions/local.mcpb.blender-lab.blender'
a=argparse.ArgumentParser();a.add_argument('--port',default='9877');a.add_argument('--probe-only',action='store_true');a.add_argument('--verify',action='store_true');a.add_argument('--review-only',action='store_true');a.add_argument('--inspect-review',action='store_true');args=a.parse_args()
 ...
 async with stdio_client(StdioServerParameters(command=str(EXT/'.venv/bin/python'),args=['-m','blmcp'],cwd=str(EXT),env={**os.environ,'BLENDER_MCP_PORT':args.port})) as(r,w):
  async with ClientSession(r,w) as s:
   ...
   if args.inspect_review:
    code="import bpy\nfrom mathutils import Vector\nsource="+repr(str(OUT/'blender/eiffel-summit-operation-rig-review.blend'))+"\nwith bpy.data.libraries.load(source,link=False) as(src,dst): dst.scenes=src.scenes\ns=dst.scenes[0]\nbpy.context.window.scene=s\nbpy.context.view_layer.update()\nrows=[]\nfor o in s.objects:\n if any(o.name.startswith(p) for p in ('summit-crown-m072-c001','summit-crown-m074-c000','summit-crown-m075-c000')):\n  vs=[o.matrix_world@Vector(v) for v in o.bound_box]\n  rows.append({'name':o.name,'parent':o.parent.name if o.parent else None,'matrix':[list(r) for r in o.matrix_world],'min':[min(v[i] for v in vs) for i in range(3)],'max':[max(v[i] for v in vs) for i in range(3)]})\nresult={'objects':rows}"
    ...
    code='import bpy\nfrom pathlib import Path\nsource='+repr(str(OUT/'blender/eiffel-summit-operation-rig.blend'))+'\nwith bpy.data.libraries.load(source,link=False) as(src,dst): dst.scenes=src.scenes\ns=dst.scenes[0]\nbpy.context.window.scene=s\nbpy.context.view_layer.update()\nresult={"file":source,"objects":len(s.objects),"meshes":sum(o.type=="MESH" for o in s.objects),"roles":sorted(o.get("wf_role") for o in s.objects if o.get("wf_role"))}'
   else:
    source=str(ROOT/'scripts/blender_eiffel_summit_operation.py');code=f'from pathlib import Path\nscope={{"__file__":{source!r}}}\nexec(compile(Path({source!r}).read_text(),{source!r},"exec"),scope)\nresult=scope["result"]'
   response=await s.call_tool('execute_blender_code',{'code':code});label='inspect-review' if args.inspect_review else 'review' if args.review_only else 'readback' if args.verify else 'build'
