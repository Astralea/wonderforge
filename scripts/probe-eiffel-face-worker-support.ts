...
const search={candidates:read('artifacts/eiffel-face-installation-2026-09-08/worker-deck-search.json')};
for(const r of read('artifacts/eiffel-face-transfer-2026-09-08/receiver-occupancy.json'))fixed.push({id:r.id,shape:eiffelConvexSolid(r.vertices,[[0,1,3,2],[4,6,7,5],[0,4,5,1],[2,3,7,6],[0,2,6,4],[1,5,7,3]])});
...
writeFileSync('artifacts/eiffel-face-installation-2026-09-08/worker-support-search.json',JSON.stringify(reports,null,2)+'\n');
for(const [i,r] of reports.entries())console.log(i,r.deck.center,r.choices.map(c=>c.filter(b=>b.hits.length===0).map(b=>b.a)));
