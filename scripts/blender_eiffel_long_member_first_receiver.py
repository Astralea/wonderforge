...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'artifacts/eiffel-real-payload-route-2026-09-08'
design=json.loads((OUT/'design.json').read_text());scene=bpy.data.scenes.new('WonderForge — long-member first-floor receiver candidate');bpy.context.window.scene=scene
...
bpy.ops.export_scene.gltf(filepath=str(OUT/'model/long-member-first-receiver.glb'),export_format='GLB',use_selection=True,use_active_scene=True,export_extras=True,export_yup=True,export_animations=False,export_lights=False,export_cameras=False)
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'blender/eiffel-long-member-first-receiver.blend'))
result=dict(scene=scene.name,objects=len(scene.objects),blend=str(OUT/'blender/eiffel-long-member-first-receiver.blend'),productionReady=False)
