// Causal A/B of the real Sydney renderer at the bright-roof orbit angle.
import { chromium } from '@playwright/test';
import { mkdir, writeFile } from 'node:fs/promises';
import { PNG } from 'pngjs';
const out='artifacts/sydney-reference-rebuild-2026-09-23/sun-ray-ab';
...
for(const t of [.5,.58,.625,.7])for(const enabled of [true,false]){
 const data=await page.evaluate(({t,enabled})=>{window.rays.update(t,t,t);window.rays.pipeline.godrays.enabled=enabled;window.rays.pipeline.render(t);return document.querySelector('#rays').toDataURL();},{t,enabled});
