...
OUT=ROOT/'artifacts/eiffel-first-floor-transfer-2026-09-08/mcp'
...
async def main():
    ...
    async with stdio_client(StdioServerParameters(command=str(EXT/'.venv/bin/python'),args=['-m','blmcp'],cwd=str(EXT))) as (r,w):
        async with ClientSession(r,w) as session:
            ...
            source=str(ROOT/'scripts/blender_eiffel_first_floor_transfer.py')
            winch_source=str(ROOT/'scripts/blender_eiffel_first_floor_transfer_winch.py')
            code=f"""from pathlib import Path
scope={{'__file__':{source!r}}}
exec(compile(Path({source!r}).read_text(),{source!r},'exec'),scope)
bridge_result=scope['result']
winch_scope={{'__file__':{winch_source!r}}}
exec(compile(Path({winch_source!r}).read_text(),{winch_source!r},'exec'),winch_scope)
result={{'bridge':bridge_result,'winch':winch_scope['result']}}
"""
