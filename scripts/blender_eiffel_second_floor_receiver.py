...
design=json.loads((OUT/'design.json').read_text());scene=bpy.data.scenes.new('WonderForge — second-floor receiving winch');bpy.context.window.scene=scene
...
cargo=group('cargo',design['sourceCargo'])
...
root=group('second-floor-station',design['transform']['translation'])
root.rotation_euler.z=math.pi/2
for o in list(scene.objects):
 if o!=root and o.parent is None:o.parent=root
cart=group('second-floor-cart',design['cart'])
box('cart-bed',[0,.29,0],[.8,.10,.8],cart,timber)
for x in (-.25,.25):
 for z in (-.38,.38):
  bpy.ops.mesh.primitive_cylinder_add(vertices=24,radius=.12,depth=.07);o=bpy.context.object;o.name='cart-wheel';o.parent=cart;o.location=(x,-z,.12);o.rotation_euler.x=math.pi/2;o.data.materials.append(iron);o['wf_role']='cart-wheel'
...
bpy.ops.export_scene.gltf(filepath=str(OUT/'model/second-floor-receiver.glb'),export_format='GLB',use_selection=True,use_active_scene=True,export_extras=True,export_yup=True,export_animations=False,export_lights=False,export_cameras=False)
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'blender/eiffel-second-floor-receiver.blend'))
result=dict(scene=scene.name,objects=len(scene.objects),blend=str(OUT/'blender/eiffel-second-floor-receiver.blend'),productionReady=False)
