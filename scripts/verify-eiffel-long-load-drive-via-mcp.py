...
OUT=ROOT/'artifacts/eiffel-long-load-main-2026-09-08/mcp'
...
async def main():
    ...
    async with stdio_client(StdioServerParameters(command=str(EXT/'.venv/bin/python'),args=['-m','blmcp'],cwd=str(EXT),env={**os.environ,'BLENDER_MCP_PORT':'9877'})) as (r,w):
        async with ClientSession(r,w) as session:
            ...
            source=str(OUT.parent/'verify-source.py')
            ...
            (OUT/'verify-result.json').write_text(json.dumps(response.model_dump(mode='json'),indent=2)+'\n')
