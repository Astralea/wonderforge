...
void main() {
  ...
  float warmBand = exp(-max(0.0, elevation) * 16.0) * uHaze;
  color = mix(color, uHorizon, warmBand * 0.32);
...
export class SydneySkyDome {
  readonly mesh: Mesh;
  private readonly geometry = new SphereGeometry(1_500, 40, 20);
  ...
  constructor() {
    this.material = new ShaderMaterial({
      ...
      fog: false,
      uniforms: {
        uZenith: { value: new Color('#2f86c8') },
        uHorizon: { value: new Color('#d8c4a8') },
        uFogColor: { value: new Color('#3a7a9c') },
        uSunDirection: { value: new Vector3(0, 1, 0) },
        uSunTint: { value: new Color('#fff6de') },
        uCloudTint: { value: new Color('#f7f3ec') },
        uCloudShadow: { value: new Color('#6e92ac') },
        uCloudOpacity: { value: 0.18 },
        uHaze: { value: 0.1 },
        uTime: { value: 0 },
      },
    });
    this.mesh = new Mesh(this.geometry, this.material);
    this.mesh.name = 'sydney-world-space-harbour-sky';
    this.mesh.frustumCulled = false;
    this.mesh.renderOrder = -100;
  }

  update(t: number, sky: SydneySkySample, sunDirection: Vector3): void {
    const uniforms = this.material.uniforms;
    (uniforms.uHorizon!.value as Color).set(sky.horizon);
    (uniforms.uZenith!.value as Color).set(sky.zenith);
    deriveSceneFogColor(sky.horizon, this.fogScratch, HARBOUR_FOG_NEUTRALIZER);
