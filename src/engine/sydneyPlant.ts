...
export type SydneyPlantKind =
  'tower-crane' | 'crawler-crane' | 'dozer' | 'dump-truck';
...
const onGround = (localX: number, localZ: number): Vec3 => {
  const { x, z } = sydneyBuildingToWorld(localX, localZ);
  return [x, sydneyTerrainHeightAt(x, z), z];
};
...
export function sydneyPlantAt(rawT: number): SydneyPlantPose[] {
  ...
  if (t >= 0.12 && t <= 0.8) {
    poses.push({
      ...
      kind: 'crawler-crane',
      position: onGround(-8, 158),
      yaw: SYDNEY_BUILDING_YAW - 0.52 + pingPong(t, 1.35, 0.18) * 0.4,
  ...
  if (t >= 0.02 && t <= 0.23) {
    ...
    poses.push({
      id: 'dozer-yard',
      kind: 'dozer',
      position: onGround(lerp(-43, 20, yardU), 145),
      yaw: SYDNEY_BUILDING_YAW + Math.PI / 2,
    ...
    poses.push({
      id: 'dozer-apron',
      kind: 'dozer',
      position: onGround(lerp(-42, 20, apronU), 128),
      yaw: SYDNEY_BUILDING_YAW + Math.PI / 2,
    ...
    poses.push({
      id: 'dozer-west',
      kind: 'dozer',
      position: onGround(-60, lerp(137, 163, westU)),
      yaw: SYDNEY_BUILDING_YAW,
  ...
  if (t >= 0.04 && t <= 0.23) {
    for (let truck = 0; truck < 3; truck += 1) {
      ...
      const x = lerp(30, -35, u);
      const z = lerp(155, 165, u);
      poses.push({
        id: `dump-truck-${truck}`,
        kind: 'dump-truck',
        position: onGround(x, z),
        yaw: SYDNEY_BUILDING_YAW + Math.atan2(-35 - 30, 165 - 155),
