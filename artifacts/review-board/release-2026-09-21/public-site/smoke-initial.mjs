...
try {
 ...
 await page.close();
 for(const [mode,width,height] of [['desktop',1440,900],['portrait',390,844],['narrow',320,844]]) {
  ...
  for(const id of ['pyramids-of-giza','stonehenge','colosseum','eiffel-tower']) {
   ...
   if(mode==='narrow')await p.screenshot({path:`${out}/narrow-${id}.png`});
   await p.getByRole('button',{name:'About this wonder',exact:true}).click();
   check(await p.getByRole('dialog').count()===1,`${mode}/${id}: info dialog missing`);
   await p.keyboard.press('Escape');
   check(await p.getByRole('dialog').count()===0,`${mode}/${id}: Escape fails to close info`);
  }
...
} finally {await browser.close();await writeFile(`${out}/smoke-results.json`,JSON.stringify(result,null,2));}
