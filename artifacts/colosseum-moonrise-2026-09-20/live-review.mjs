...
for(const [mode,width,height] of [['desktop',1280,720],['mobile',390,844]]){
 ...
 for(const t of [.06,.48,.670278,.8,.88,.94,.48,.88,1]){
  ...
  if(t===.88||t===.93)await page.screenshot({path:`artifacts/colosseum-moonrise-2026-09-20/production/${mode}-ui-${t}.png`});
 }
 ...
 await page.screenshot({path:`artifacts/colosseum-moonrise-2026-09-20/${mode}-live-ending.png`});
 ...
 await page.screenshot({path:`artifacts/colosseum-moonrise-2026-09-20/${mode}-live-return.png`});
 results.push({mode,errors,shots,continuous,replayValue,home});await page.close();
}
await writeFile('artifacts/colosseum-moonrise-2026-09-20/production/live-review.json',JSON.stringify(results,null,2));
