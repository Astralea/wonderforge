...
const root = 'artifacts/colosseum-moonrise-2026-09-20';
const DEG = Math.PI / 180;
type Body = 'sun' | 'moon';
type Row = { jd: number; az: number; el: number; radius: number };
const grids = Object.fromEntries((['sun', 'moon'] as const).map(body => {
  const result = JSON.parse(readFileSync(`${root}/ephemeris/grid-jun21-23-${body}.response.json`, 'utf8')).result as string;
...
function frame(t: number, aspect: number) {
  ...
  return Object.fromEntries((['sun', 'moon'] as const).map(body => {
    ...
    return [body, { t, seconds: t * 60, hour: (b.jd - 1750449.5) * 24, az: b.az, el: b.el, x: p.x, y: p.y, radiusDegrees: radius, fullyFramed: Math.abs(p.x) + margin / aspect <= 1 && Math.abs(p.y) + margin <= 1 && direction.dot(camera.getWorldDirection(new Vector3())) > 0, aboveHorizon: b.el > 0 }];
...
for (const aspect of [16 / 9, 390 / 844, 320 / 844]) {
  ...
  output.push({ aspect, angularScale: COLOSSEUM_CELESTIAL_ANGULAR_SCALE, visible, frames: [.0, .4, .76, .8, .84, .87, .88, .90, .94, .97, 1].map(t => frame(t, aspect)) });
