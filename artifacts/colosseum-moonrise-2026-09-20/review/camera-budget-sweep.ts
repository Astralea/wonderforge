...
const results = [];
for (const aspect of [1.6, 16 / 9, 390 / 844, 320 / 844, 375 / 667, .6, .71]) {
  ...
  for (let frame = 0; frame <= 3600; frame++) {
    ...
    world.update(colosseumFilmAt(t).constructionT, light, new Vector3(1, 1, 1), sampleColosseumSky(t), camera);
    let count = 0;
...
world.dispose();
writeFileSync('artifacts/colosseum-moonrise-2026-09-20/review/camera-budget-sweep.json', JSON.stringify(results, null, 2));
