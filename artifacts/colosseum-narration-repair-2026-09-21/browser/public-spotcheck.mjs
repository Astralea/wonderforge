...
try{
 for(const [mode,width,height]of [['desktop',1440,900],['mobile',390,844]]){
  for(const track of tracks){
   ...
   await page.goto(`https://wonderforge.pages.dev/#/wonder/${track.wonderId}`,{waitUntil:'networkidle'});
   ...
   if(mode==='desktop'){
    ...
    const assets=await Promise.all(track.beats.filter(b=>['colosseum-seating','colosseum-titus'].includes(b.id)).map(async beat=>{
     const clip=manifest.find(c=>c.captionId===beat.id),response=await page.request.get(`https://wonderforge.pages.dev${clip.src}`),body=await response.body();
   ...
   if(mode==='mobile')await page.getByRole('button',{name:'Close chapters',exact:true}).click();
   await page.getByRole('button',{name:'Turn narration on',exact:true}).click();await page.waitForTimeout(180);
   const clips=[];
   for(const beat of track.beats.filter(b=>['colosseum-seating','colosseum-titus'].includes(b.id))){
    ...
    await page.screenshot({path:`${out}/${mode}-${beat.id}-playing.png`});
    let waitError=null;
