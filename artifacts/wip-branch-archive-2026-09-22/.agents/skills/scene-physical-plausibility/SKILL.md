...
description: "Physical rules a WonderForge construction scene must obey — support, occupancy, right-of-way, continuity of appearance, path validity, ground contact, instance hygiene, labour kinematics, and camera honesty — plus how to verify each one numerically instead of by eye. Use when placing props, scheduling construction, adding earthworks or routes, authoring crews or camera paths, or when a reviewer reports clipping, floating, popping, chorus-line workers, a frozen opening shot, or objects growing through each other."
---

# Scene Physical Plausibility

WonderForge is a diorama, never a rigid-body simulation. It never needs
rigid-body physics — it needs to never show the viewer something that could
not happen. A single
...
- Authoring crews, rigs, scaffold raise/strike, or cinematic camera keys.
- A review reports clipping, floating, sinking, popping, chorus-line labour,
  a frozen opening camera, or "objects overlap".
...
Support is a numeric contact constraint, not a label. Transform the relevant
contact through the object's rotation and compare it with the declared support
surface: lower stone extent for ground/sled/ramp/rail/masonry, runner underside
for the road, heel for an upright pivoting into a pit, and lintel soffit for a
crib or stone joint. The object origin is never evidence of contact; for a
horizontal upright, subtracting `dimensions[1] / 2` is the wrong axis.
...
Terrain, road, ramp, crib, rail, and joint height must come from a shared pure
sampler consumed by engine state and renderer geometry. Never leave local turf
relief in a private renderer helper. Apply carrier clearance exactly once:
state must document whether sled-bed, skid, or crib lift is already included in
the structural transform, and the renderer must obey that convention.
...
- **Nudge** (`pushOutOfSiteWorks`) — for props whose exact position is free:
  tents, crates, shade frames. Push along the shallowest axis so the
...
Sample terrain at the object's current `(x,z)`, including centimetre-scale turf
relief outside a level working floor. Route waypoint height, a global zero
plane, or an endpoint-only check never proves contact along the travelled
path.
...
Contract tests should also assert that every slot below `mesh.count` has a
finite authored matrix and that no unwritten identity transform appears at the
world origin.
...
### 10. Transfer quality, not mechanics

Use failures in either reference scene to strengthen both contracts. Carry
Stonehenge's surface-matched interpolation, contact binding, horizon/fog
honesty, and target-owned sky discipline into Giza. Carry Giza's human
yardstick, typed keep-outs/site clearance, procedural material recipes,
bounded living-site density, and course/support occupancy tests into
Stonehenge. Keep the mechanisms target-specific: never Giza ramp semantics in
Stonehenge or Colosseum, and never Stonehenge pit/heel semantics in Giza.
Never copy Colosseum crane/scaffold Y-scale tricks onto Giza poles, and never
glue crews to a rising deck in any scene.

## 11. Labour kinematics — people work, never pose as a ring

Crews are part of the physical claim. A planted spectator ring, a chorus-line
orbit, or a worker glued to a rising platform is the same class of failure as
a floating stone: the eye reads contradiction.

- Bind every visible person to an operation or a named site job. Gait, lean,
  and rig spin are functions of phase progress. A bob on a planted pose is
  never labour.
- Unique jobs and occupancy from seeded per-id salts. Never occupy every
  station around an oval. Never `sin(t * k + i * phase)` as a shared ring
  walk. Never slingers at `local * TAU + i / n * TAU`.
- Never ride a rising platform. Climb onto a deck that already exists. Cap
  worker Y by the visible stack height. Scaffold poles are ground-rooted
  segments of authored length; never fake raise/strike by scaling Y.
- Wagon wheels roll with haul travel. A treadwheel spins from hook *vertical*
  travel and holds still during a pure slew.
- Camera motion is physical honesty too. If the spec promises a turn from
  frame one, azimuth must leave the opening heading immediately. Never freeze
  the intro with duplicate azimuth keys. Never easeInOutQuad azimuth when that
  ease zeros the derivative at t=0.

See `.agents/skills/wonderforge-scene-builder/references/labour-camera.md`.

## How to verify

Never eyeball it. Perspective hides a 2-unit overlap and invents others.

**Measure contact residuals first.** For each support kind, sample every phase
boundary and several interior points (at least 25%, 50%, and 75%). Compute
`contactY - supportY` from the transformed geometry or authored heel/soffit,
not from the object origin. A rigid contact should stay within about 0.03 world
units unless the scene spec documents another tolerance; report positive gaps
as floats and negative gaps as burial. Separately read the production renderer
matrices to catch an adapter that reapplies a sled or crib lift. Cover multiple
stones, routes, lanes, and both reference scenes when shared support code
changes. Prove the test is load-bearing by temporarily restoring the known bad
offset or origin-versus-surface calculation and watching it fail.
...
whose starts ascend never gives ascending *end* times once durations vary —
...
Labour and camera have numeric proofs too: gait/yaw/radius spread among
peers; climber count far below station count; climber Y never equal to a
rising `deckY` during raise; azimuth at an early `t` has left the opening
heading when the spec forbids a hold. Reproduce presentation bugs under live
playback from t=0, never only by loading a debug time.

For Stonehenge, use `artifacts/vertex-wonder-ref/stonehenge-construction.png`
or `.jpg` when present, without waiting for it to arrive. Its useful pixel
tests are narrow contact shadows, turf or chalk interrupted at seated bases,
sled runners visibly on the grass, timber cribs meeting the lintel soffit
beside the support pair, workers reading as a human yardstick, and a high-angle
dusk composition with mechanism foreground, monument midground, rolling downs,
and a clouded blue sky.
